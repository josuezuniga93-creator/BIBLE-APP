...
import { analyzeSermon } from "./lib/api";
...
export default function Home() {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      let body: Parameters<typeof analyzeSermon>[0];

      if (activeTab === "youtube") {
        setIsTranscriptPhase(true);
        setLoadingMsg(t.fetchingTranscript);
        body = { youtube_url: youtubeUrl.trim(), language: lang, max_claims: maxClaims };
        ...
        setIsTranscriptPhase(false);
        setLoadingMsg(t.analyzingSermon);
        body = { text: text.trim(), language: lang, max_claims: maxClaims };
      }

      const data = await analyzeSermon(body);
  ...
  }, [isReady, activeTab, youtubeUrl, text, lang, t, maxClaims, history, saveHistory]);
      ...
      <main className="max-w-4xl mx-auto px-6 py-12 space-y-6">
        ...
        {!analysis && (
          <div className="space-y-4 fade-in print:hidden">
            ...
            {/* Depth selector */}
            <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3 flex items-center gap-4">
        ...
        {analysis && !loading && (
          <div ref={resultsRef} className="space-y-6 fade-in">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 pt-2 pb-4">
              <button
                onClick={handleReset}
                className="group flex items-center gap-2 py-3 px-8 rounded-xl font-semibold border border-white/15 bg-white/[0.04] hover:bg-white/[0.08] text-white/50 hover:text-white/80 transition-all duration-200 flex-1 justify-center sm:flex-none"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 py-3 px-6 rounded-xl font-semibold border border-violet-500/25 bg-violet-500/[0.07] hover:bg-violet-500/[0.14] text-violet-400 hover:text-violet-300 transition-all duration-200 print:hidden flex-1 justify-center sm:flex-none text-sm"
              >
                <svg
                  className="w-4 h-4"
                  fill="none"
